...
Claude Code는 `CLAUDE_CODE_MAX_OUTPUT_TOKENS`를 매 요청 `max_tokens` 예약으로 사용하므로, 이 env에는 `max_output_tokens` 능력치를 넣지 않는다. 현재 Claude harness 정책은 reservation `32000`, tokenizer headroom `8192`, minimum input `32768`이다. 런처 계산식:
...
이 분리는 Claude Code에만 적용한다. Codex/OpenCode/Goose가 capability metadata로 쓰는 `max_output_tokens` anchor는 바꾸지 않는다.
...
| Goose `GOOSE_CONTEXT_LIMIT` | launch env 주입(`adapterConfig.env`의 `{{limits.context}}` 토큰) | 다음 launch |
...
| `effective_input` | 현재 architecture가 입력 예산으로 다루는 값. Claude LiteLLM은 출력 예약과 tokenizer headroom을 차감한 값이다. |
...
- Claude output reservation 정책이 각 tier에서 최소 입력 여유를 남기는지 확인한다.
- OpenCode output이 32000 기본 clamp에 걸릴 수 있음을 경고한다.
- Goose가 현재 `GOOSE_CONTEXT_LIMIT`만 주입하고 role-specific planner/lead/worker limit은 주입하지 않음을 경고한다.
