...
capability metadata itself — hence the two live in different files with
different owners.
...
### 4d. Gemma4-12B — local Claude haiku tier (oMLX)
- **Claude:** `haiku` tier → `Gemma4-12B-omlx` (a promoted first-class
  registry entry, not a discovered route, so the tier survives reinstall).
  It is fully local/free, so Claude Code background/subagent calls routed to
  haiku do not spend provider budget. The exact cap is intentionally
  conservative; check the current value with `ai-litellm model limits
  Gemma4-12B-omlx`.

### 4e. Qwen3.6-27B — local, thinking helps (oMLX)
...
- **Route:** `Qwen3.6-27B-omlx` remains a promoted/local route that can be
  selected explicitly (`claude-litellm --proxy Qwen3.6-27B-omlx`) or used as a
  live local smoke target. It is not the current Claude `haiku` alias.
- **Thinking:** stays **on** — qualification showed 27B follows instructions fine in the
  heavy harness with thinking enabled.

### 4f. Qwen3.6-35B-A3B — local, thinking must be off (oMLX)
